...
type EventItem = { id:number; title:string; category:string; type:string; day:string; month:string; date:string; time:string; venue:string; address:string; distance:string; price:string; image:string; description:string };
  ...
  return <div className="min-h-screen bg-cream text-ink font-sans">
    ...
    <main className="pt-[78px]">
      ...
      <section className="mx-auto max-w-[1220px] px-[5vw] py-10">
        ...
        <div className="flex gap-2 overflow-auto border-b border-[#dce0d9] pb-8"><span className="mr-2 whitespace-nowrap self-center font-mono text-[10px] uppercase tracking-wide text-[#89928c]">Explore by</span>{categories.map(c => <button key={c} onClick={() => setCategory(c)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-[11px] font-semibold transition ${category === c ? "border-ink bg-ink text-white" : "border-[#dce0d9] text-[#65706a] hover:bg-ink hover:text-white"}`}>{c}</button>)}</div>
        ...
        {filtered.length ? <div ref={gridRef} className="mt-7 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map(e => <EventCard key={e.id} event={e} isSaved={saved.includes(e.id)} onSave={toggleSave} onOpen={setSelected} />)}</div> : <div className="py-20 text-center"><p className="font-mono text-xs text-[#89928c]">Nothing by that name, yet.</p><button className="mt-4 text-xs font-bold text-coral" onClick={() => setQuery("")}>Clear search</button></div>}
        <section id="saved" className="mt-24 border-t border-[#dce0d9] pt-11"><div className="mb-7 flex items-end justify-between"><div><p className="mb-4 font-mono text-[10px] uppercase tracking-[1.3px] text-[#8a948e]">Your shortlist</p><h2 className="text-3xl font-bold tracking-[-2px]">Saved for later <span className="rounded-full bg-[#fbe0d8] px-2 py-1 font-mono text-[11px] text-coral">{saved.length}</span></h2></div><p className="hidden text-[11px] leading-relaxed text-[#87908a] sm:block">Keep the good ones close.<br />We’ll remember them for you.</p></div>{savedEvents.length ? <div className="grid gap-4 md:grid-cols-3">{savedEvents.map(e => <EventCard compact key={e.id} event={e} isSaved onSave={toggleSave} onOpen={setSelected} />)}</div> : <p className="font-mono text-[10px] text-[#969f99]">Your saved events will show up here.</p>}</section>
      </section>
        ...
        <Newsletter email={email} setEmail={setEmail} subscribed={subscribed} onSubscribe={() => { if (email.includes("@")) setSubscribed(true); }} />
        <Footer onNavigate={navigate} />
      </>}
    </main>
    {page !== "discover" && <Footer onNavigate={navigate} />}
